...
// Added options to restore the last details in model and current file name 
import { useEffect, useState } from "react";
...
const defaultGallery: galleryImages[] = [
  { name: "Lion", image: lion, descr: "The majestic lion, the king of the jungle." },
  { name: "Peacock ", image: Peacock, descr: "A beautiful peacock with a colorful feathers" },
  { name: "Cat", image: cat, descr: "A cute and curious cat with soft fur." },
  { name: "Butterfly", image: Butterfly, descr: "A Beautiful Butterfly in a flower." },
  { name: "Bird", image: Bird, descr: "A beautiful bird on a flower." },
  { name: "Waterfall", image: Waterfall, descr: "A waterfall in somewhere." },
...
export default function Gallerys() {
  ...
  // Save only user-added images to localStorage
  useEffect(() => {
  ...
  const handleCancel = () => {
    // Backup current input so it can be restored later
    setBackupData({
      name: newImageName,
      descr: newImageDescr,
      file: newImageFile,
  ...
  const openAddNew = () => {
    // Restore last inputs if user cancelled before
    if (backupData.name || backupData.descr || backupData.file) {
      setNewImageName(backupData.name);
      setNewImageDescr(backupData.descr);
      setNewImageFile(backupData.file);
    }
    setShowModal(true);
  };
              ...
              <div className="modal-body">
                ...
                {editIndex !== null && (
                  <p className="text-muted">
                    <strong>Current Image:</strong> {gallery[editIndex].name}
                  </p>
                )}
